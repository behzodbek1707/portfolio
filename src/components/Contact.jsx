...
const channels = [
  { icon: '✉', label: 'bek.abdumutalov12@gmail.com',       href: 'mailto:bek.abdumutalov12@gmail.com' },
  { icon: '⌥', label: 'github.com/behzodbek1707',         href: 'https://github.com/behzodbek1707' },
  { icon: 'in', label: 'linkedin.com/in/behzodbek-abdumutalov',   href: 'https://www.linkedin.com/in/behzodbek-abdumutalov/' },
  { icon: '✈', label: '@idle_coder',               href: 'https://t.me/idle_coder' },
]
...
export default function Contact() {
  ...
  const onFocus = (e) => {
    ...
    e.target.style.boxShadow   = '0 0 0 3px rgba(0,240,255,0.07)'
  }
  const onBlur = (e) => {
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              {channels.map(c => (
                <a key={c.label} href={c.href} target="_blank" rel="noreferrer"
                  ...
                  <span style={{ fontSize: '0.78rem', color: 'var(--text)' }}>{c.label}</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="card reveal reveal-d2" ref={rightRef}
            ...
            <div>
              ...
              <input type="text" placeholder="Your name" value={form.name}
                onChange={set('name')} style={inp} onFocus={onFocus} onBlur={onBlur} />
            </div>

            <div>
              <label style={{ fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)', display: 'block', marginBottom: '0.4rem' }}>Email</label>
              <input type="email" placeholder="your@email.com" value={form.email}
                onChange={set('email')} style={inp} onFocus={onFocus} onBlur={onBlur} />
            </div>
            ...
            <div>
              ...
              <textarea rows={4} placeholder="Tell me about your project..." value={form.message}
                onChange={set('message')} style={{ ...inp, resize: 'none' }} onFocus={onFocus} onBlur={onBlur} />
            </div>

            {status === 'error' && (
              <div style={{ fontSize: '0.72rem', color: '#f87171', padding: '0.6rem 0.9rem', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 3 }}>
                ⚠ {errMsg}
              </div>
            )}
            ...
            <button onClick={handleSubmit} disabled={status === 'sending' || status === 'sent'}
              ...
              style={{
                ...
                background: status === 'sent' ? '#0aff9d' : undefined,
              }}>
            ...
            {status === 'sent' && (
              <p style={{ fontSize: '0.72rem', color: 'var(--accent)', marginTop: '-0.5rem' }}>
                You'll receive an auto-reply shortly. I'll be in touch soon!
              </p>
            )}
          </div>
